...
args = getResolvedOptions(sys.argv,['S3_BUCKET','deal_data_key','price_data_key'])
...
deal_data_path='s3://%s/%s' % (S3_BUCKET,args['deal_data_key'])
price_data_path='s3://%s/%s' % (S3_BUCKET,args['price_data_key'])
...
def calculate_mean_price_each_second(price):
    logger.info('Calculating mean price for each second')
...
def get_historical_price(deals_with_price,mean_price_each_second):
    logger.info('Getting historical price for each deal')
...
def calculate_relative_price(all_data_absolute):
    logger.info('Calculating the price relative to MID for each deal')
...
def get_labels(deals_with_price):
    logger.info('Getting labels for each deal')
...
def create_datasets(all_data,labels):
    ...
    logger.info("training data length: %s, training label length: %s" %(len(datasets['training']['data']),len(datasets['training']['labels'])))
    logger.info("validation data length: %s, validation label length: %s" %(len(datasets['validation']['data']),len(datasets['validation']['labels'])))
    logger.info("test data length: %s, test label length: %s" %(len(datasets['test']['data']),len(datasets['test']['labels'])))
...
def save_dataset(dataset,key):
    ...
    buf.seek(0)
    session.resource('s3').Bucket(S3_BUCKET).Object('%s_data.io' % key).upload_fileobj(buf)
    logger.info('%s dataset saved to S3' % key)
...
def save_all_data_as_csv(all_data):
    logger.info('Saving all data as CSV for verification')
    ...
    with open('all_data.csv', "rb") as f:
        s3.upload_fileobj(f, S3_BUCKET,'all_data.csv')
        logger.info('CSV saved to S3')
